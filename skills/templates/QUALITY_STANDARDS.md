...
> **These apply to every component built with `/ds-build` or `/ds-generate`.** They are not per-job settings — they are Kido's baseline acceptance criteria. Per-job rules live in `REQUIREMENTS.md`; this file holds the rules that never change between jobs.
>
> Items marked ⚙️ are checked automatically by the `/ds-build` validator. Items marked 👁️ require designer sign-off before handoff.
...
- Whether a component is responsive or fixed-width must be stated explicitly in REQUIREMENTS.md (under Component-Specific Constraints).
